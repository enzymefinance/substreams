    ...
    pub mod functions {
                ...
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.vault_proxy),
                        ),
        ...
        impl DeployVaultProxy {
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.vault_lib),
                        ),
                        ethabi::Token::Address(ethabi::Address::from_slice(&self.owner)),
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.vault_accessor),
                        ),
        ...
        for GetFundDeployerForVaultProxy {
            fn output(data: &[u8]) -> Result<Vec<u8>, String> {
                Self::output(data)
            }
        }
        ...
        for HasExecutableMigrationRequest {
        ...
        impl SetCurrentFundDeployer {
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.next_fund_deployer),
                        ),
                    ],
                );
                let mut encoded = Vec::with_capacity(4 + data.len());
                    ...
                    &[
                        ethabi::Token::Uint(
                            ethabi::Uint::from_big_endian(
                                match self.next_timelock.clone().to_bytes_be() {
        ...
        impl SetNominatedOwner {
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.next_nominated_owner),
                        ),
                    ],
                );
                let mut encoded = Vec::with_capacity(4 + data.len());
        ...
        impl SetSharesTokenSymbol {
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[ethabi::Token::String(self.next_symbol.clone())],
                );
                let mut encoded = Vec::with_capacity(4 + data.len());
                encoded.extend(Self::METHOD_ID);
                encoded.extend(data);
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ...
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.next_vault_accessor),
                        ),
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.next_vault_lib),
                        ),
                        ethabi::Token::Bool(self.bypass_failure),
                    ],
                );
